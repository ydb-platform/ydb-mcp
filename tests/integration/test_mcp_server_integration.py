...
async def test_simple_query(mcp_server):
    ...
    assert (
        isinstance(result, list) and len(result) > 0 and "text" in result[0]
    ...
    assert (
        len(parsed["result_sets"]) == 1
    ), f"Expected 1 result set, got {len(parsed['result_sets'])}"
...
async def test_create_table_and_query(mcp_server):
    ...
    try:
        ...
        assert (
            ...
            and len(create_result) > 0
            and "text" in create_result[0]
        ...
        assert (
            isinstance(insert_result, list)
            and len(insert_result) > 0
            and "text" in insert_result[0]
        ...
        assert (
            isinstance(query_result, list) and len(query_result) > 0 and "text" in query_result[0]
...
async def test_multiple_resultsets_with_tables(mcp_server):
    ...
    try:
        ...
        assert (
            isinstance(setup_result, list) and len(setup_result) > 0 and "text" in setup_result[0]
        ...
        assert (
            isinstance(insert_result2, list)
            and len(insert_result2) > 0
            and "text" in insert_result2[0]
        ...
        assert (
            isinstance(join_result, list) and len(join_result) > 0 and "text" in join_result[0]
        ...
        assert (
            len(parsed["result_sets"]) == 1
        ), f"Expected 1 result set for join, got {len(parsed['result_sets'])}"
...
async def test_data_types(mcp_server):
    ...
    assert "rows" in first_result, f"No rows in result: {first_result}"
    assert (
        len(first_result["columns"]) == 2
    ), f"Expected 2 columns, got {len(first_result['columns'])}"
    assert len(first_result["rows"]) == 1, f"Expected 1 row, got {len(first_result['rows'])}"

    # Verify column names
    assert (
        "value" in first_result["columns"]
    ), f"Expected column 'value', got {first_result['columns']}"
    assert (
        "text" in first_result["columns"]
    ), f"Expected column 'text', got {first_result['columns']}"
...
async def test_all_data_types(mcp_server):
    ...
    ), f"Expected string_value to be 'Hello, World!', got {string_value}"
    ...
    ), f"Expected json_value to be '{{'key': 'value'}}', got {json_value}"
    ...
    if isinstance(date_value, str):
        ...
        assert parsed_date == datetime.date(
            2023, 7, 15
        ), f"Expected date_value to be 2023-07-15, got {parsed_date}"
    else:
        assert isinstance(
            date_value, datetime.date
        ), f"Expected date_value to be datetime.date, got {type(date_value)}"
        assert date_value == datetime.date(
            2023, 7, 15
        ), f"Expected date_value to be 2023-07-15, got {date_value}"
    ...
    if isinstance(datetime_value, str):
        ...
        assert (
            parsed_dt == expected_datetime
        ), f"Expected datetime_value to be {expected_datetime}, got {parsed_dt}"
    else:
        assert isinstance(
    ...
    if isinstance(timestamp_value, str):
        ...
        assert (
            parsed_ts == expected_timestamp
        ), f"Expected timestamp_value to be {expected_timestamp}, got {parsed_ts}"
    else:
        assert isinstance(
    ...
    if isinstance(interval_value, str):
        ...
        if interval_value.endswith("s"):
            ...
            assert (
                parsed_interval.total_seconds() == expected_interval.total_seconds()
        ...
        assert isinstance(
            interval_value, datetime.timedelta
        ), f"Expected interval_value to be datetime.timedelta, got {type(interval_value)}"
        assert (
            interval_value.total_seconds() == expected_interval.total_seconds()
    ...
    if isinstance(decimal_value, str):
        ...
        assert parsed_decimal == Decimal(
            "123.456789"
        ), f"Expected decimal_value to be Decimal('123.456789'), got {parsed_decimal}"
    else:
        assert isinstance(
            decimal_value, Decimal
        ), f"Expected decimal_value to be Decimal, got {type(decimal_value)}"
        assert decimal_value == Decimal(
            "123.456789"
    ...
    assert isinstance(
        string_list, list
    ), f"Expected string_list to be a list, got {type(string_list)}"
    expected = ["a", "b", "c"]
    ...
    assert isinstance(
        simple_struct, dict
    ), f"Expected simple_struct to be a dict, got {type(simple_struct)}"
    assert (
        "a" in simple_struct and "b" in simple_struct
    ...
    assert (
        simple_struct["b"] == "x"
    ), f"Expected simple_struct['b'] to be 'x', got {simple_struct['b']}"

    # Dictionary containers
    string_to_int_dict = get_value("string_to_int_dict")
    assert isinstance(
        string_to_int_dict, dict
    ...
    assert (
        string_to_int_dict == expected_dict or string_to_int_dict == expected_bytes_dict
    ...
    assert isinstance(
        list_of_structs, list
    ), f"Expected list_of_structs to be a list, got {type(list_of_structs)}"
    assert (
        len(list_of_structs) == 3
    ), f"Expected list_of_structs to have 3 items, got {len(list_of_structs)}"

    # Check first item in list of structs
    first_struct = list_of_structs[0]
    assert isinstance(
        first_struct, dict
    ), f"Expected first_struct to be a dict, got {type(first_struct)}"
    assert first_struct == {
        "id": 1,
        "name": "Alice",
    ...
    assert isinstance(
        struct_with_list, dict
    ), f"Expected struct_with_list to be a dict, got {type(struct_with_list)}"
    assert struct_with_list == {
        "collection_name": "users",
        "ids": [1, 2, 3],
        "active": True,
    }, f"Expected struct_with_list to be {{'collection_name': 'users', 'ids': [1, 2, 3], 'active': True}}, got {struct_with_list}"

    # Complex dict
    complex_dict = get_value("complex_dict")
    assert isinstance(
        complex_dict, dict
    ), f"Expected complex_dict to be a dict, got {type(complex_dict)}"
    ...
    assert (
        complex_dict == expected_complex_dict or complex_dict == expected_bytes_complex_dict
    ...
    assert isinstance(
        nested_list, list
    ), f"Expected nested_list to be a list, got {type(nested_list)}"
    ...
    ), f"Expected nested_list to be {expected_nested_list}, got {nested_list}"

    # Tuple containers
    mixed_tuple = get_value("mixed_tuple")
    assert isinstance(
        mixed_tuple, (list, tuple)
    ), f"Expected mixed_tuple to be a list or tuple, got {type(mixed_tuple)}"
    ...
    assert (
        mixed_tuple == expected_tuple
    ), f"Expected mixed_tuple to be {expected_tuple}, got {mixed_tuple}"
...
async def test_list_directory_integration(mcp_server):
    ...
    assert isinstance(
        parsed["items"], list
    ), f"Expected items to be a list, got {type(parsed['items'])}"
...
async def test_ydb_status_integration(mcp_server):
    ...
    assert (
        parsed["ydb_connection"] == "connected"
